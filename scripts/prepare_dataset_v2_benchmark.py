"""
音频数据集预处理脚本 V2 (Benchmark 版本)
⚠️ 使用非常小的batch进行性能测试和调试
...
# 并行处理 (BENCHMARK 配置)
NUM_WORKERS = 2      # CPU workers 数量（benchmark用小值）
DAC_BATCH_SIZE = 2   # DAC 编码批处理大小（benchmark用小值）
FILE_BATCH_SIZE = 4  # 文件批处理大小（benchmark用小值）
USE_MULTI_GPU = True  # 是否使用多GPU并行
MAX_FILES = 10       # ⚠️ BENCHMARK: 只处理前10个文件
...
def get_audio_files():
    ...
    # 排序确保每次运行顺序一致
    audio_files.sort()
    print(f"✅ 找到 {len(audio_files)} 个音频文件 (BENCHMARK 限制: 最多 {MAX_FILES} 个)")
    return audio_files
...
def load_and_resample_audio(filepath):
    ...
    try:
        ...
        t_numpy = time.time() - t_numpy_start

        t_total = time.time() - t0

        # BENCHMARK: 打印详细时间
        print(f"    [⏱️  {filename[:30]:30s}] "
              f"总计:{t_total*1000:5.1f}ms | "
              f"加载:{t_load*1000:4.1f}ms | "
              f"单声道:{t_mono*1000:4.1f}ms | "
              f"HR重采样:{t_hr*1000:4.1f}ms | "
              f"LR重采样:{t_lr*1000:4.1f}ms | "
              f"转numpy:{t_numpy*1000:4.1f}ms")
...
def batch_encode_dac(audio_list, dac_model, device):
    ...
    t_tensor = time.time() - t_tensor_start
    ...
    # BENCHMARK: 打印详细时间
    print(f"      [⏱️  DAC Batch={len(audio_list)}] "
          f"总计:{t_total*1000:5.1f}ms | "
          f"转tensor:{t_tensor*1000:4.1f}ms | "
          f"Pad/Stack:{t_pad*1000:4.1f}ms | "
          f"移GPU:{t_gpu*1000:4.1f}ms | "
          f"编码:{t_encode*1000:4.1f}ms | "
          f"转CPU:{t_cpu*1000:4.1f}ms")
...
def main():
    print("=" * 70)
    print("⏱️  音频数据集预处理 V2 (BENCHMARK 版本 - 性能测试)")
    print("=" * 70)
    print("\n⚠️  BENCHMARK 配置:")
    print(f"  • 只处理前 {MAX_FILES} 个文件")
    print(f"  • NUM_WORKERS = {NUM_WORKERS} (减少以便观察)")
    print(f"  • FILE_BATCH_SIZE = {FILE_BATCH_SIZE}")
    print(f"  • DAC_BATCH_SIZE = {DAC_BATCH_SIZE}")
    print(f"  • 每个步骤都有详细时间戳输出")
    print("\n改进:")
    print("  1. ✅ 全局均值/标准差归一化")
    print("  2. ✅ 保存为高效 .pt 格式")
    print("  3. ✅ 每首歌一个文件")
    print("  4. ✅ 两阶段处理 (统计 → 转换)")
    print(f"  5. ✅ 详细时间戳输出（用于性能分析）")
    print("=" * 70)
