#!/usr/bin/env python3
"""
DAC Overfit Test for JaT-AudioSR V2 (DESTRUCTIVE VERSION)
⚠️ 破坏性测试：在生成音频前随机重置模型权重
用途：验证模型是否真的加载了训练好的权重

如果生成的音频质量很差 → 说明确实加载了权重（被破坏后变差）
如果生成的音频质量没变 → 说明可能没有正确加载权重
"""
...
def generate_audio(model, dac_model, lr_latent, hr_mean, hr_std, num_steps=50, device='cuda'):
    ...
    print("✅ 已随机重置所有 Linear 和 LayerNorm 层的权重")
    print("   如果生成音频质量很差 → 说明之前确实加载了训练好的权重")
    print("   如果生成音频质量没变 → 说明可能没有正确加载权重")
    print("⚠️ " * 30 + "\n")
    # ==================== 破坏性代码结束 ====================
...
def main():
    ...
    print("=" * 70)
    print("⚠️  DAC OVERFIT TEST - JaT-AudioSR V2 (DESTRUCTIVE VERSION)")
    print("=" * 70)
    print(f"\n⚠️  破坏性测试版本")
    print(f"   音频长度: {args.duration}s")
    print(f"   ⚠️  警告：生成音频前会随机重置模型权重")
    print(f"   目的：验证模型是否真的加载了训练好的权重")
    print("=" * 70)
    ...
    if success or losses[-1] < 0.2:
        ...
        try:
            ...
            sf.write(output_dir / 'generated_audio_v2_DESTRUCTIVE.wav', generated_audio, 48000)
            print(f"  ✅ 保存生成音频: {output_dir}/generated_audio_v2_DESTRUCTIVE.wav")
            print(f"     ⚠️  注意：这是用随机重置权重后的模型生成的")
            ...
            print(f"    5. outputs/generated_audio_v2_DESTRUCTIVE.wav - ⚠️ 破坏性测试生成（权重已重置）")

            print(f"\n  🎧 听音频对比质量!")
            print(f"\n  ⚠️  预期结果：")
            print(f"     - 如果 generated_audio_v2_DESTRUCTIVE.wav 质量很差（噪音）")
            print(f"       → 说明训练确实有效，权重被正确加载了")
            print(f"     - 如果质量没有明显变差")
            print(f"       → 说明可能没有正确加载训练好的权重")
    ...
    if success:
        ...
        print("  ✅ 训练pipeline完整")
        print(f"\n⚠️  下一步：对比音频文件")
        print("   正常版本: outputs/generated_audio_v2.wav")
        print("   破坏版本: outputs/generated_audio_v2_DESTRUCTIVE.wav")
        print("\n   如果破坏版本质量很差 → 训练确实有效")
        print("   如果破坏版本质量没变 → 可能有问题")
    else:
        print("⚠️  训练阶段失败")
        print("   检查loss曲线和组件")
