"""
JaT-AudioSR V1 Training Script (Enhanced Version)

V1 Improvements:
1. ✅ Dropout 0.1 (Attention + MLP)
2. ✅ DropPath 0.05 (Stochastic Depth, 线性递增)
3. ✅ Conditional Noise 0.05 (基于batch std的自适应噪声)
4. ✅ Phase 1 TensorBoard指标 (GradNorm, SNR_dB, PredictionMean/Std, Val Loss Std)
5. ✅ 智能检查点管理 (时间戳文件夹 + auto-resume)

对比V1:
- V1: 仅Dropout，无DropPath，无Conditional Noise
- V2: 完整正则化体系，防止过拟合

前向兼容:
- DropPath默认0时为Identity，可加载V1权重
- Conditional Noise仅训练时添加，验证时关闭
"""
...
class TrainConfig:
    ...
    # --- 训练超参 ---
    batch_size = 84         # 单卡 Batch (总 Batch = 48)
    lr = 5e-5               # 稳健学习率
    weight_decay = 0.1
    ...
    # --- 保存与日志 ---
    save_dir_base = "checkpoints/v1_full_run"  # 基础目录
    log_dir_base = "runs/v1_full_run"          # 基础目录
...
def u_shaped_timestep_sampling(batch_size, device, alpha=0.5):
    """V1 核心: U-shaped Timestep Sampling"""
...
def main():
    # 1. 解析命令行参数
    parser = argparse.ArgumentParser(description='JaT V1 Training with DDP')
    ...
    if is_master:
        ...
        print("="*60)
        print(f"🚀 JaT V1 Training Started")
...
#```bash
#tensorboard --logdir=runs/v1_full_run --port 6006 --bind_all