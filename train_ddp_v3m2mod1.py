"""
JaT-AudioSR V3M2 MOD1 Training Script (Charbonnier Loss + Unified Config)

V3M2 MOD1 Improvements:
1. 🔥 Charbonnier Loss 替代 MSE (更鲁棒，防止梯度爆炸)
2. 🔥 统一Loss配置区 (所有权重集中管理)
📚 参考文献

  - Charbonnier Loss:
    - https://neptune.ai/blog/pytorch-loss-functions (CVPR 2017)
...
V3M2 Base Improvements (继承):
1. ✅ RMSNorm (替代LayerNorm) - 更快、更稳定、28层深网络标配
2. ✅ Dropout 0.1 (Attention + MLP)
3. ✅ DropPath 0.05 (Stochastic Depth, 线性递增)
4. ✅ CFG训练 (10% condition dropout) - Audio SR标配，推理时可调guidance
5. ✅ Conditional Noise 0.02 (降低至2%，CFG已提供足够正则化)
6. ✅ BFloat16精度 (W7900原生支持，深网络更稳定)
7. ✅ Phase 1 TensorBoard指标 (GradNorm, SNR_dB, PredictionMean/Std, Val Loss Std)
8. ✅ 智能检查点管理 (时间戳文件夹 + auto-resume)

Modern LLM Stack:
- RMSNorm (LLaMA 3, Mistral标配)
- GQA (Grouped-Query Attention)
- RoPE (Rotary Position Embeddings)
- DropPath (Stochastic Depth)
- CFG训练 (Classifier-Free Guidance)
- BFloat16 (AMD RDNA3原生支持)

对比V2:
- V2: LayerNorm + GELU
- V3: RMSNorm + GELU + CFG + BF16
"""
...
from src.models.jat_audiosr_v3 import JaT_AudioSR_V3

# ==============================================================================
# 🎯 Loss Functions (MOD1: Charbonnier Loss)
# ==============================================================================
...
class TrainConfig:
    # --- 硬件与并行 ---
    seed = 42
    num_workers = 16         # 降低worker数量避免内存爆炸（90MB×16workers=巨大）
    ...
    # --- 正则化增强 (V3) ---
    cfg_dropout_prob = 0.1        # 🔥 CFG训练：10% condition dropout (DiT标准)
    condition_noise_ratio = 0.02  # Conditional Noise: 2%相对噪声 (降低，CFG已足够)
    use_adaptive_noise = True     # 使用自适应噪声（基于batch std）

    # ==============================================================================
    # 🔥 MOD1: 统一Loss配置区 (Centralized Loss Configuration)
    # ==============================================================================
    #
    ...
    # --- Loss 权重配置 (可独立调节) ---
    reconstruction_weight = 1.0        # 主重建损失权重 (通常固定为1.0)
    ...
    # --- 训练超参 ---
    batch_size = 28         # 单卡 Batch (总 Batch = 48)
    lr = 5e-5               # 稳健学习率
    weight_decay = 0.1
    ...
    # --- 保存与日志 ---
    save_dir_base = "checkpoints/v3m2mod1_full_run"  # 🔥 MOD1基础目录
    log_dir_base = "runs/v3m2mod1_full_run"          # 🔥 MOD1基础目录
...
def main():
    ...
    # 5. 模型初始化 (V3: 带RMSNorm)
    model = JaT_AudioSR_V3(**cfg.model_params).to(local_rank)
    ...
    if is_master:
        print("=" * 60)
        print("🔥 MOD1: Charbonnier Loss + Unified Config")
        print("=" * 60)
    ...
    for epoch in range(start_epoch, cfg.num_epochs):
        ...
        for hr, lr in train_loader:
            ...
            # V3: 使用BFloat16精度（W7900原生支持，28层深网络更稳定）
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                # A. 归一化 (Normalize)
                hr_norm = (hr - hr_mean) / hr_std
                lr_norm = (lr - lr_mean) / lr_std

                # A1. Conditional Noise Augmentation (先加噪，防止过度依赖LR输入)
                # 降低至2%，因为CFG已提供足够正则化
                ...
                # A2. CFG Condition Dropout (🔥 V3新增：Classifier-Free Guidance训练)
                # 最后执行！确保null token是纯0（没有被noise污染）
                # Sample-level masking：batch中每个样本独立决定是否dropout
                B = hr.shape[0]
                cfg_mask = torch.rand(B, device=hr.device) < cfg.cfg_dropout_prob  # [B]
                cfg_mask = cfg_mask.view(B, 1, 1)  # [B, 1, 1] 用于broadcast
                lr_norm = lr_norm * (~cfg_mask).float()  # 被mask的样本置零
                ...
                # E. 主重建损失 (MOD1: Charbonnier or MSE)
                if cfg.use_charbonnier_loss:
...
def validate(model, loader, hr_mean, hr_std, lr_mean, lr_std, device):
    ...
    with torch.no_grad():
        for hr, lr in loader:
            ...
            # 验证集一定要用 autocast 保持一致性 (V3: BFloat16)
            with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                pred_x0 = model(z_t, t, lr_norm)

                # MOD1: 主重建损失 (Charbonnier or MSE)
